...
A lot of problems arose from the path structure in our project, such as accessing the appropriate configuration files during both training and inference. This simply took a lot of debugging to overcome. We also faced some challenge with authentication of Google Cloud and Wandb when running Docker containers. This was solved partly by using private keys and by making our GCS buckets public. Docker was also a challenge initially as we initially weren't accustomed to it and spent a lot of time waiting for Docker images to be built in order to run them. Also, debugging when problems accured insdie a docker container in the cloud was challenging as the logs provides by Google weren't specific enough. In addition to the long time for building a container bugs when running a container used up a lot of time. Writing a robust set of unittests also posed a challenge as the code base changed frequently, so the tests had to be general enough to also apply to future changes. For the tests to be more robust we could have defined the general layout of the project more specifically, so that these defined cornerstones could be checked for. This could then protect the core elements of the project.
